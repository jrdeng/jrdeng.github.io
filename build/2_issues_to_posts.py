...
def usage():
    ...
    print('\t-f|--file FILE\t\tissue json file which generated by jrdeng/query-issues-action or 2_query_issues.py')
    print('\t-d|--dir POST_DIR\tpost dir to store *.md files')
...
def write_hexo_body(md, issue):
    md.write('{}\n\n'.format(issue['body']))


def write_comments(md, owner, repo, issue):
    if not "<!-- more -->" in issue['body']:
        md.write('<!-- more -->\n\n')
...
def generate_hexo_post(owner, repo, issue_list, output_dir):
    ...
    for issue in issue_list:
        ...
        with open('{}{}{}'.format(output_dir, os.sep, file_name), 'w+') as md:
            write_hexo_header(md, issue)
            write_hexo_body(md, issue)
...
def main():
    ...
    generate_hexo_post(owner, repo, issue_list, post_dir)
